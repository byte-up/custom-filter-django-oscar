...
class SimpleProductSearchHandler(MultipleObjectMixin):
    ...
    def get_search_context_data(self, context_object_name, request=None):
        # Set the context_object_name instance property as it's needed
        # internally by MultipleObjectMixin
        self.context_object_name = context_object_name
        if request:
            if request.GET.get('new', None) == 'on':
                self.object_list = self.object_list.order_by('-date_created')
            if request.GET.get('promotion', None) == 'on':
                self.object_list = self.object_list.filter(is_discountable=True)                
